...
def model_builder(hp):
    ...
    hp_lambda = hp.Choice('l2_lambda', values=[1e-06, 1e-05, 5e-05, 1e-4, 5e-4, 1e-3, 5e-3, 1e-2, 5e-2, 1e-1, 1.0])
...
def tune_model(X_train, y_train):
    ...
    layer {best_hps.get('hidden')}, the optimal learning rate for the optimizer
    is {best_hps.get('learning_rate')} and the optimal lambda for the L2 regularizer
    is {best_hps.get('l2_lambda')}.
    """)
...
def main():
    ...
    # TODO: model.fit already has val split!!! No need to split CV?

    tune_model(data, target)
